...
void LowLevelListHandler(unsigned int chNo, unsigned int wayNo, void (*funPtr)(unsigned int, unsigned int))
{
	...
	if (waitWayCnt != USER_WAYS)
		if (!V2FIsControllerBusy(chCtlReg[chNo]))
		{
			...
			if (wayPriorityTablePtr->wayPriority[chNo].readTriggerHead != WAY_NONE)
			{
				...
				LowLevelListHandler(chNo, wayNo, SelectiveGetFromNandReadTriggerList);
			}
			...
			if (wayPriorityTablePtr->wayPriority[chNo].writeHead != WAY_NONE)
			{
				...
				LowLevelListHandler(chNo, wayNo, SelectiveGetFromNandWriteList);
			}
			if (wayPriorityTablePtr->wayPriority[chNo].readTransferHead != WAY_NONE)
			{
				...
				LowLevelListHandler(chNo, wayNo, SelectiveGetFromNandReadTransferList);
			}
		}
}
